...
unsigned char *Canny_Edge_detector(unsigned char *image, int width, int height, int channels, int paddig_size, int *new_width_out, int *new_height_out, int *new_channels){
    ...
    //Step 1.5(optional): Turning image from RGB(or RGBA) into Gray
    if (channels >= 3){
        int a,b;
        unsigned char *gray = rgb_to_grayscale(blurred, new_width, new_height, channels, &a, &b, new_channels); 
        free(blurred);
        blurred = gray;
    }
    ...
    for(int i = 0; i < new_width * new_height; i++){
        if(supressed[i] < 50){
            supressed[i] = 0;
        }else if(supressed[i] >= 50 && supressed[i] < 100){
            supressed[i] = 100;
        }else if(supressed[i] >= 100){
            supressed[i] = 255;
        }
    }
